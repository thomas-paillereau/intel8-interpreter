...
char *getInstruction(char *text, int offset, int *returnSize)
{
    ...
    if (startsBy(bin, "1100011___000") || startsBy(bin, "100000")
        || startsBy(bin, "1111011") || startsBy(bin, "1010100"))
    {
        if (!w)
    ...
    if (strcmp(reg, "") && strcmp(rm, "")) // Inst with reg && r/m
    {
        if (d && strcmp(sInst, "xchg"))
            asprintf(ind, "%s%s %s, %s%s%s%s%s%s\n", pre, sInst, sReg, brack1,
                     sEA, plus, sEAAddi2, sEAAddi1, brack2);
        else
            asprintf(ind, "%s%s %s%s%s%s%s%s, %s\n", pre, sInst, brack1, sEA,
                     plus, sEAAddi2, sEAAddi1, brack2, sReg);
    }
    else if (strcmp(rm, "")) // Inst with r/m only
    {
        if (s != -1)
        {
            ...
            {
                if ((startsBy(bin, "1100011") || startsBy(bin, "100000")
                     || startsBy(bin, "1111011") || startsBy(bin, "1010100"))
                    && !strcmp(sWData, "") && tmp < 16)
                    asprintf(ind, "%s%s %s%s%s%s%s%s%s, %01x\n", pre, sInst,
                             byte, brack1, sEA, plus, sEAAddi2, sEAAddi1,
                             brack2, abs(tmp));
                else if ((startsBy(bin, "1100011") || startsBy(bin, "100000")
                    ...
                    asprintf(ind, "%s%s %s%s%s%s%s%s%s, %x\n", pre, sInst, byte,
                             brack1, sEA, plus, sEAAddi2, sEAAddi1, brack2,
                             tmp);
            }
        }
        else if (v != -1)
            ...
            asprintf(ind, "%s%s %s%s%s%s%s%s%s\n", pre, sInst, byte, brack1,
                     sEA, plus, sEAAddi2, sEAAddi1, brack2);
        else if (d)
            asprintf(ind, "%s%s %s%s%s%s%s%s%s, %s%s\n", pre, sInst, byte,
                     brack1, sEA, plus, sEAAddi2, sEAAddi1, brack2, sWData,
                     sData);
        else
            asprintf(ind, "%s%s %s%s%s, %s%s%s%s%s%s\n", pre, sInst, byte,
                     sWData, sData, brack1, sEA, plus, sEAAddi2, sEAAddi1,
                     brack2);
    }
...
char **disassembler(char *text, int *sizeDisa)
{
    ...
    *sizeDisa = size;
    ...
    while (i < size)
    {
        ...
        if ((r = contains(s2, 'D')) != -1)
            getData(bin, &data,
                    !strcmp(wdata, "") ? nSpace * 8 : (nSpace - 1) * 8);
        ...
        if (strcmp(reg, "") && strcmp(rm, "")) // Inst with reg && r/m
        {
            if (d && strcmp(sInst, "xchg"))
                ...
                asprintf(ind, "%s%s %s%s%s%s%s%s, %s\n", pre, sInst, brack1,
                         sEA, plus, sEAAddi2, sEAAddi1, brack2, sReg);
        }
        else if (strcmp(rm, "")) // Inst with r/m only
        {
            if (s != -1)
...
// Function to check if a string starts by another string given in the
// arguments. The use of the character '_' permits to skip some in the original
// string
int startsBy(char *string, char *start)
{
    int i = 0;
...
// Check if a character is present in the string, made to decrypt the
// architecture of an instruction
int contains(char *s, char c)
{
    int n = 0;
    for (int i = 0; s[i]; i++)
...
// Function to change the size of an instructions to take all the bytes
// necessary
void changeByteSize(char *code, int index, char **s, int newByteSize)
{
    free(*s);
...
void getInst2(char *bin, char **printedString, char **sInst, int size)
{
    if (!(same(*sInst, "push") || same(*sInst, "add") || same(*sInst, "sub")
          || same(*sInst, "ssb") || same(*sInst, "dec") || same(*sInst, "neg")
          || same(*sInst, "cmp") || same(*sInst, "shl") || same(*sInst, "and")
          || same(*sInst, "test") || same(*sInst, "or") || same(*sInst, "xor")
          || same(*sInst, "call") || same(*sInst, "jmp")))
        return;
