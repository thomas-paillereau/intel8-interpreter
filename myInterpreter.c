...
int main(int argc, char *argv[])
{
    ...
    if (file == NULL)
    {
        printf("The file \"%s\" was not found or opened, please check if you "
               "used the right name.\n",
               argv[1]);
        return 1;
    }
    ...
    // Getting the information from the code and the data of the given
    // executable
    int sizeCode;
...
void interpreter(CPU *cpu, char *code, int m)
{
    ...
    while (!quit)
    {
        ...
        if (m && !halt)
            printf(
                "%04hx %04hx %04hx %04hx %04hx %04hx %04hx %04hx %c%c%c%c %s",
                cpu->ax, cpu->bx, cpu->cx, cpu->dx, cpu->sp, cpu->bp, cpu->si,
                cpu->di, OF, SF, ZF, CF, line);
        ...
        if (m && !halt)
        {
            ...
            if (*seg && *(seg + 1) && strchr(seg[1], '['))
                oldValueDisplay(cpu, seg[1],
                                8 <= sFindReg(seg[0]) && sFindReg(seg[0]) < 16);
            printf("\n");
        }
        ...
        else if (!strcmp(inst, "adc"))
        {
            ...
            UpdateLastReg(cpu, seg[0]);
            UpdateOF(cpu, value1, value2 + value3, value1 + value2 + value3,
                     "add");
            UpdateSF(cpu, value1 + value2);
            UpdateZF(cpu, value1 + value2);
...
// Segments a string for each time there is the given character "separ". If the
// character is '+', the '-' character is also counted
char **segCut(char *line, char separ)
{
...
// Detects where should the value be gotten (memory / register / immediate) and
...
// Sets the value, automaticly finding where to put the value (register /
// memory)
void setValue(CPU *cpu, char *text, short value, char *src)
{
...
// Takes a string of an hexadecimal number and returns the corresponding
// interger
int HexStrToUInt(char *s)
{
    int minus = 1;
